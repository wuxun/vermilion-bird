...
from ember_agent.workflow.spec import WorkflowNodeSpec, WorkflowSpec
...
class PatternStage(WorkflowNodeSpec):
    """One stage in a collaboration pattern."""


class CollaborationPattern(WorkflowSpec):
...
register_pattern(
    CollaborationPattern(
        ...
        stages=[
            PatternStage(id="pro", role="executor", task="Argue FOR: {user_task}"),
            PatternStage(
                id="con",
                role="critic",
...
register_pattern(
    CollaborationPattern(
        ...
        stages=[
            PatternStage(id="creator", role="planner", task="Create first draft: {user_task}"),
            PatternStage(
                id="critic",
                role="critic",
    ...
    CollaborationPattern(
        ...
        stages=[
            PatternStage(
                ...
                role="executor",
                parallel=2,
                task="Analyze one aspect of: {user_task}",
                collect="numbered",
            ),
        ],
        aggregator_role="synthesizer",
...
register_pattern(
    CollaborationPattern(
        ...
        stages=[
            PatternStage(id="stage_0", role="planner", task="Plan the approach for: {user_task}"),
            PatternStage(
                id="stage_1",
                role="executor",
                task="Execute the plan from stage_0: {stage_0.result}",
                depends_on=["stage_0"],
            ),
            PatternStage(
                id="stage_2",
                role="synthesizer",
                task="Polish and finalize: {stage_1.result}",
                depends_on=["stage_1"],
            ),
        ],
    )
...
register_pattern(
    CollaborationPattern(
        ...
        stages=[
            PatternStage(
                id="creator", role="executor", task="Create initial output for: {user_task}"
            ),
            PatternStage(
                id="critic",
                role="critic",
    ...
    try:
        ...
        for name, spec in patterns.items():
            ...
            pattern = CollaborationPattern(
                name=name,
                description=spec.get("description", f"Pattern: {name}"),
                stages=stages,
                aggregator_role=spec.get("aggregator_role"),
