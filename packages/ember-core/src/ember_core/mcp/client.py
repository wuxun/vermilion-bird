"""MCPClient — connect to a single MCP server (stdio or SSE transport).

Uses the mcp Python SDK. Handles connection lifecycle, capability loading,
and tool/resource access.
"""
...
class MCPClient:
    ...
    async def _connect_stdio(self) -> bool:
        ...
        cmd_dir = os.path.dirname(
            os.path.abspath(os.path.expanduser(self.config.command))
        )
        if cmd_dir:
        ...
        logger.info(
            f"启动 stdio MCP 服务器: command={self.config.command}, "
            ...
            async with self._connection_lock:
                    ...
                    try:
                        ...
                        async with stdio_client(server_params) as (read, write):
                            ...
                            async with ClientSession(read, write) as session:
                                ...
                                logger.info(
                                    f"MCP 服务器 {self.config.name} 连接成功，保持运行..."
                                )
                ...
                try:
                    await asyncio.wait_for(
                        self._connection_ready.wait(), timeout=self.config.timeout
                    )
                    if self._stopping:
    ...
    def connection_ready(self) -> None:
    ...
    async def _connect_sse(self) -> bool:
        ...
        try:
            async with self._connection_lock:
                ...
                try:
                    await asyncio.wait_for(
                        self._connection_ready.wait(), timeout=60
                    )
                    if self._stopping:
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any]) -> Any:
        ...
        try:
            ...
            if result.isError:
                error_msg = (
                    result.content[0].text if result.content else "未知错误"
                )
