...
- 只展示 `uncertain` 的 Outbox 记录，呈现 effect key、工具、参数、来源 Run 与审批 ID；
- 人工核实外部系统后，记录“确认已成功”“确认未发生”或“允许安全重试”；
- 成功/失败结论同步收敛 Outbox、ActionProposal 和 Run，避免三个状态源互相矛盾；
- 审计说明、操作者、对账时间与可选结果均持久化，历史记录不会被覆盖；
- “允许安全重试”只把 Outbox 恢复为可执行状态，不会在 GUI 内立即重放副作用。
...
应用启动时还会运行幂等修复：若进程在 Outbox 对账结论落盘后、关联 Run/审批同步前崩溃，
会依据已持久化结论补齐关联状态。
