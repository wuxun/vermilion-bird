...
- options: 2-3 个方向，每个只需 label（名称）和 description（说明）
  id 自动分配 A/B/C，无需手动指定
  - label: 方向名称（要有张力，让人想点）
  - description: 30字以内 — 具体聊什么，聊完能带来什么
...
class ProactiveAgent:
        ...
        for attempt in range(len(RETRY_HINTS)):
            ...
            try:
                ...
                if response and len(response.strip()) > 20:
                    items = self._parse_curated(response)
                    if items:
                        logger.info(
                            f"LLM 精选完成 (attempt {attempt + 1}): "
                            f"{len(items)} 条"
                        )
                        return items
                    else:
                        logger.warning(
                            f"精选解析为空 (attempt {attempt + 1}): "
                            f"{response[:100]}"
                        )
                else:
                    logger.warning(
                        f"精选返回空 (attempt {attempt + 1}, "
                        f"len={len(response or '')})"
                    )
            except Exception as e:
                logger.error(f"LLM 精选失败 (attempt {attempt + 1}): {e}")
    ...
    def _get_feishu_adapter(self):
        """懒初始化飞书适配器，与 NotificationService 模式一致。"""
        ...
        try:
            ...
            if app_id and app_secret:
                adapter = FeishuAdapter(self._app, app_id, app_secret)
                self._app._feishu_adapter = adapter
                return adapter
        except Exception as e:
            logger.warning(f"初始化飞书适配器失败: {e}")
        return None

    def _push_text(self, text: str):
        """Push plain text to Feishu and GUI."""
        if not text.strip():
            logger.warning("_push_text: 文本为空，跳过推送")
            return
                ...
                if adapter:
                    ...
                    if recent:
                        ...
                        if receive_id:
                            ...
                            logger.info("新闻精选已推送到飞书")
                            pushed = True
                else:
                    logger.info("_push_text: 飞书适配器未初始化，跳过飞书推送")
            except Exception as e:
        ...
        if not pushed:
            # 没有任何推送渠道可用，至少打日志
            logger.warning(
                f"_push_text: 无可用推送渠道，文本内容:\n{text[:500]}"
            )

    # ----------------------------------------------------------------
    # 讨论管线 (mode="discussion")
    # ----------------------------------------------------------------

    def _run_discussion(self):
        """Phase 2: read digest → generate discussion card."""
        ctx = self._build_context()
    ...
    def _fetch_rss(self) -> str:
        ...
        for url in feeds:
            try:
                # 用 requests 拉取（支持代理 + 超时），再交给 feedparser 解析
        ...
        if snippets:
            logger.info(f"RSS 采集完成: {len(snippets)} 条摘要")
    ...
    def _generate_card_with_digest(self, ctx: dict):
        ...
        Called by _run_discussion(). Pushes card on success.
        """
        from llm_chat.proactive.prompts import DISCUSSION_PROMPT

        # 构建 user message
        base_sections = ["## 背景信息", f"当前时间：{ctx.get('time', '')}"]

        # 今日精选优先展示
        if ctx.get("digest_summary"):
            base_sections.append("\n### 🌐 今日精选（主要素材）")
            base_sections.append(ctx["digest_summary"])
