...
from llm_chat.decision.engine import DecisionEngine
from llm_chat.decision.schema import DecisionCard, CardType, DecisionOption
...
class ProactiveAgent:
    ...
    def _generate_card(self) -> Optional[DecisionCard]:
        ...
        sections.append("")
        sections.append("---")
        sections.append("请生成 JSON 格式的话题建议卡片：")
        ...
        try:
            ...
            engine = DecisionEngine(self._app.client)
            card = engine._parse_response(
                response,
                card_type=CardType.DECISION,
            )
            return card
        except Exception as e:
            logger.error(f"生成话题卡片失败: {e}")
            return None
