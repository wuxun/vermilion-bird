...
class CreateTaskPlanTool(BaseTool):
    def __init__(self, base_dir: str = ".", work_dir: str = "."):
        self._base_dir = Path(base_dir).resolve()
        self._work_dir = Path(work_dir).resolve()
        self._tasks_file = self._work_dir / ".todo_tasks.json"
    ...
    def execute(
        self, task_name: str, task_description: str, steps: List[Dict[str, Any]]
    ) -> str:
        try:
            ...
            for step in steps:
                formatted_steps.append(
                    {
                        "step_id": step.get(
                            "step_id", f"step_{len(formatted_steps) + 1}"
                        ),
            ...
            logger.info(
                f"创建任务计划: {task_id} - {task_name}, 共 {len(steps)} 个步骤"
            )
            ...
            for i, step in enumerate(formatted_steps, 1):
                ...
                result += (
                    f"  {i}. [{step['step_id']}] {step['description']}{deps_str}\n"
                )
...
class UpdateStepProgressTool(BaseTool):
    ...
    def get_parameters_schema(self) -> Dict[str, Any]:
        return {
            "type": "object",
            "properties": {
                "step_id": {"type": "string", "description": "要更新的步骤ID"},
                "progress": {
                    "type": "integer",
    ...
    def execute(
        self, step_id: str, progress: int, note: str = "", status: str = "in_progress"
    ) -> str:
        try:
            task_data = self._load_tasks()
            ...
            if note:
                target_step["notes"].append(
                    {"time": datetime.now().isoformat(), "content": note}
                )
...
class GetTaskStatusTool(BaseTool):
    ...
    def execute(self, task_id: str = "") -> str:
        try:
            ...
            completed_steps = sum(
                1 for s in target_task["steps"] if s["status"] == "completed"
            )
            total_steps = len(target_task["steps"])
            overall_progress = (
                sum(s["progress"] for s in target_task["steps"]) // total_steps
                if total_steps > 0
                else 0
            )
        ...
        try:
            ...
            for step in active_task["steps"]:
                ...
                if step["status"] == "in_progress":
                    ...
                    unmet_deps = [
                        d for d in dependencies if d not in completed_step_ids
                    ]
            ...
            all_completed = all(
                s["status"] == "completed" for s in active_task["steps"]
            )
            if all_completed:
                result += f"\n🎉 所有步骤已完成！任务可以结束了。\n"
...
class CompleteStepTool(BaseTool):
    ...
    def get_parameters_schema(self) -> Dict[str, Any]:
        return {
            "type": "object",
            "properties": {
                "step_id": {"type": "string", "description": "要完成的步骤ID"},
                "completion_note": {
                    "type": "string",
    ...
    def execute(
        self, step_id: str, completion_note: str = "", actual_output: str = ""
    ) -> str:
        try:
            task_data = self._load_tasks()
            ...
            completed_step_ids = set(
                s["step_id"] for s in active_task["steps"] if s["status"] == "completed"
            )
            ...
            completed_count = sum(
                1 for s in active_task["steps"] if s["status"] == "completed"
            )
            ...
            logger.info(
                f"步骤完成: {step_id}, 任务进度: {completed_count}/{total_count}"
            )
    ...
    def execute(self, include_completed: bool = True) -> str:
        try:
            ...
            for task in tasks:
                ...
                completed_steps = sum(
                    1 for s in task["steps"] if s["status"] == "completed"
                )
...
class SwitchTaskTool(BaseTool):
    ...
    def get_parameters_schema(self) -> Dict[str, Any]:
        return {
            "type": "object",
            "properties": {
                "task_id": {"type": "string", "description": "要切换到的任务ID"}
            },
            "required": ["task_id"],
        }
...
class TodoManagerSkill(BaseSkill):
    ...
    def get_tools(self) -> List[BaseTool]:
        return [
            CreateTaskPlanTool(self._base_dir, self._work_dir),
            UpdateStepProgressTool(self._base_dir, self._work_dir),
            GetTaskStatusTool(self._base_dir, self._work_dir),
            GetNextStepTool(self._base_dir, self._work_dir),
            CompleteStepTool(self._base_dir, self._work_dir),
            ListTasksTool(self._base_dir, self._work_dir),
            SwitchTaskTool(self._base_dir, self._work_dir),
        ]
    ...
    def on_load(self, config: Dict[str, Any]) -> None:
        ...
        self.logger.info(
            f"TodoManagerSkill loaded with base_dir: {self._base_dir}, work_dir: {self._work_dir}"
        )
