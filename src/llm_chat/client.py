...
class LLMClient:
    def __init__(self, config: Config):
        ...
        logger.info(
            f"初始化 LLMClient: protocol={config.llm.protocol}, model={config.llm.model}, base_url={config.llm.base_url}"
        )
    ...
    def chat(
        ...
        **kwargs,
        ...
        logger.info(
            f"发送聊天请求: message_length={len(message)}, history_count={len(history)}, has_system_context={system_context is not None}"
        )
    ...
    def chat_stream(
        ...
        **kwargs,
        ...
        logger.info(
            f"发送流式聊天请求: message_length={len(message)}, history_count={len(history)}, has_system_context={system_context is not None}"
        )
    ...
    def chat_stream_with_tools(
        ...
        **kwargs,
        ...
        logger.info(
            f"开始带工具的流式聊天: tools={[t['function']['name'] for t in tools]}, max_iterations={max_iterations}"
        )
        ...
        for iteration in range(max_iterations):
            ...
            for result in tool_results:
                logger.info(
                    f"处理工具结果: tool_call_id={result.get('tool_call_id')}, content_type={type(result.get('content'))}, content_is_none={result.get('content') is None}"
                )
                ...
                if i == self.config.llm.max_retries - 1:
                    ...
                    if hasattr(e, "response") and e.response is not None:
                        try:
                            ...
                            error_msg = f"{error_msg}\n响应内容: {e.response.text}"
                    logger.error(
                        f"API 请求失败(重试 {i + 1}/{self.config.llm.max_retries}): {error_msg}"
                    )
                    raise Exception(f"API 请求失败: {error_msg}")
    ...
    def _log_request_details(
        self, url: str, data: Dict, messages: List[Dict], kwargs: Dict
    ):
        """打印请求详情日志"""
        logger.info(f"{'=' * 60}")
        ...
        **kwargs,
        ...
        for iteration in range(max_iterations):
            ...
            if not self.protocol.has_tool_calls(result):
                response_text = self.protocol.parse_chat_response(result)
                logger.info(
                    f"聊天完成: iterations={iteration + 1}, response_length={len(response_text)}"
                )
                return response_text

            assistant_message = self.protocol.get_assistant_message_from_response(
                result
            )
            current_messages.append(assistant_message)
            ...
            for tool_call in tool_calls:
                logger.info(
                    f"工具调用: {tool_call.name}, 参数: {json.dumps(tool_call.arguments, ensure_ascii=False)[:100]}..."
                )
                ...
                if self._tool_registry.has_tool(tool_call.name):
                    try:
                        ...
                        if tool_result is None:
                            ...
                            logger.info(
                                f"工具 {tool_call.name} 执行成功, 结果长度: {len(tool_result)}"
                            )
